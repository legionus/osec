...
init_digest(void) {
	...
	if ((gcry_version = gcry_check_version(GCRYPT_VERSION)) == NULL)
		retval = osec_error("WARNING: libgcrypt with version %s or above needed\n",
			GCRYPT_VERSION);

	// Disable secure memory warnings
	if ((err = gcry_control(GCRYCTL_DISABLE_SECMEM_WARN)) > 0)
		retval = osec_error("WARNING (GCRYCTL_DISABLE_SECMEM_WARN): %s\n",
			gcry_strerror(err));

	// Initialize secure memory
	if ((err = gcry_control(GCRYCTL_INIT_SECMEM, 16384, 0)) > 0)
		retval = osec_error("WARNING (GCRYCTL_INIT_SECMEM): %s\n",
			gcry_strerror(err));

	if ((err = gcry_control(GCRYCTL_INITIALIZATION_FINISHED, 0)) > 0)
		retval = osec_error("WARNING (GCRYCTL_INITIALIZATION_FINISHED): %s\n",
			gcry_strerror(err));
